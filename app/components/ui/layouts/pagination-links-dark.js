...
function PaginationCard({
  ...
  return (
    <span
      className={`h-8 w-8 rounded flex items-center justify-center cursor-pointer tr-eo
          ${
            it?.label != pd.current_page &&
            "bg-base-300 hover:bg-primary active:scale-75"
          }
          ${
            it?.label == pd.current_page &&
            "bg-primary hover:scale-125 text-base-100"
          }
      `}
      onClick={onClick}
    >
